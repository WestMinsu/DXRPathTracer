...
def directional_reflectance(
    ...
    brdf = evaluate_brdf(material, roughness, view, directions)
    return np.sum(brdf * directions[:, 2, None] * weights[:, None], axis=0)
...
def run_white_furnace(directions: np.ndarray, weights: np.ndarray) -> tuple[list[dict], dict]:
    ...
    for material in MATERIALS:
        for roughness in (0.1, 0.2, 0.35, 0.65, 0.8):
...
def run_sampling_convergence(
    ...
    for case_index, (name, material, roughness, n_dot_v) in enumerate(cases):
...
def write_korean_summary(
    ...
    lines = [
        ...
        "",
        "Mitsuba 또는 RTXPT 결과를 사용하지 않고 현재 HLSL 공식을 CPU에서 독립적으로 반구 적분했습니다.",
        "",
        "## 이전 실패 원인과 수정",
        "",
    ...
    lines.extend([
        ...
        "## 검증 설정",
        "",
        f"- 결정론적 반구 적분: {mu_samples} x {phi_samples}",
        f"- 최종 Monte Carlo 표본: {mc_samples}",
        "- 최소 GGX alpha: 0.001 (최소 perceptual roughness 약 0.0316)",
        "- 상세 데이터: property_tests.csv, white_furnace.csv, pdf_consistency.csv, sampling_convergence.csv",
        "",
    ])
